...
; Create second file
	...
	SVC	@REW
	JP	NZ,DERR
; Write page of directory
	LD	B,0
GY1	XOR	A
	CALL	PUT
	DJNZ	GY1		; Loop until written
; Encode file
...
;;;
$SH3:	LD	DE,FCB2		; Rewind output file
	LD	BC,0
	SVC	@POSN		; Position to beginning
	JP	NZ,DERR
; Write directory
	LD	HL,DIR		; Directory
	LD	(FCB2+3),HL	; Put in FCB
	SVC	@WRITE		; Write sector
	JP	NZ,DERR
	SVC	@CLOSE
	JP	NZ,DERR
...
CCHARS	LD	DE,0		; Zero counter
	JR	CBRA1
CC	CALL	GET		; Get byte
	JP	NZ,CEND		; See if end
CC1	OR	A		; Loop if byte is zero
	JP	Z,CC
	DEC	A
	CP	128		; Is it above 128?
	JP	NC,CC		; Loop if so
	PUSH	AF		; Save character
	LD	A,B		; See if last character
	CP	']'-1
	JR	Z,CBRACK
	CP	'\'-1
	JR	Z,CSLASH
...
; Check for numbers
	...
	JR	NZ,CCC1
CCC0	LD	B,A
CCC1	CALL	CCHAR		; Increment character counter
	INC	E		; Increment byte counter
	JP	NZ,CC		; Loop until done
	INC	D
	EX	DE,HL
	CALL	DPERC		; Display percent
	EX	DE,HL
	JP	CC		; Loop until done
CEND	CP	28		; Is it end of file?
	JP	NZ,DERR		; Disk error if not
	CALL	DDPERC		; Display done
	RET
; Bracket
CBRACK	POP	AF		; Restore character
	JR	CBRA2		; Jump ahead
; Slash
CSLASH	POP	AF		; Restore character
CBRA1	CALL	GET		; Get number
	JP	NZ,CEND		; End if end
CBRA2	LD	B,0
	CP	'9'+1		; Is it within range?
	JR	NC,CC1		; Loop back if not
	CP	'0'
	JR	C,CC1
	JR	CBRA1		; Loop until done
;---
; Count character
; A => character
;---
CCHAR	LD	HL,COUNT0	; Count buffer
	ADD	A,A		; Double it
	ADD	A,L
	LD	L,A		; Move to point
; Increment character
...
; Find lowest
;---
FLOW	LD	HL,COUNT0	; Count table
	LD	DE,0FFFFH
	LD	A,L
	EX	AF,AF'
FLO1	LD	A,L		; See if end
	CP	254
	RET	Z
	LD	C,(HL)		; Put in BC
	INC	L
	...
	PUSH	HL
	OR	A
	SBC	HL,BC		; See if greater
	POP	HL
	EX	DE,HL
	JP	C,FLO1		; Loop if greater
	PUSH	BC		; Move to DE
	POP	DE
	EX	AF,AF'
	LD	A,L
	EX	AF,AF'
	JP	FLO1		; Loop until done
;---
; Get character
; NZ if error
;---
...
COUNT0	DC	128*2,0		; Count buffer (all letters)
STRING	EQU	$
	END	START
